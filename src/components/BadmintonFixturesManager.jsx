...
export const BadmintonFixturesManager = ({
  ...
  onStartLiveStream = null,
  onStopLiveStream = null,
  isLiveStreamActive = false,
}) => {
  // Navigation level: 'tournaments' | 'draw' (Tournament Fixtures list is always Level 1)
  ...
  const [activeLiveMatchId, setActiveLiveMatchId] = useState(() => {
    try {
      return localStorage.getItem('badminton-live-stream-match-id') || ''
    } catch {
      return ''
    }
  })
  ...
  useEffect(() => {
    if (typeof isLiveStreamActive === 'boolean') {
      setIsStreamActive(isLiveStreamActive)
    }
  }, [isLiveStreamActive])
  ...
  useEffect(() => {
    ...
    return () => window.removeEventListener('storage', syncLiveState)
  }, [])
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', minWidth: '160px' }}>
                    {isPublicView ? (
                      <button
                        ...
                        style={{ padding: '10px 14px', fontSize: '12.5px', cursor: 'pointer', textAlign: 'center', width: '100%', fontWeight: '800', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: '6px' }}
                      >
                        <span>🎯 View Official Fixtures →</span>
                      </button>
                    ) : (
                      <>
                        <button
                        ...
                        {(() => {
                            ...
                            return (
                              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '6px', width: '100%' }}>
                                <button
                                  ...
                                  title={`TV Live Stream is Active. Click to view/reopen for "${formatTournamentName(match.matchName)}"`}
                                >
                          ...
                          return (
                            <button
                              ...
                              onClick={(e) => {
                                ...
                                if (onStartLiveStream) {
                                  ...
                                  try {
                                    ...
                                    localStorage.setItem('badminton-live-stream-match-id', String(match.id))
                                    window.dispatchEvent(new Event('storage'))
                                  } catch (err) {}
                                }
                              ...
                              title={`Open Dedicated TV Live Screen in New Tab for "${formatTournamentName(match.matchName)}"`}
                            >
                              <span>📺 TV Live Cast</span>
                            </button>
                          )
                        })()}
                            ...
                            >
                              <span
                                style={{
                                  ...
                                  height: '8px',
                                  borderRadius: '50%',
                                  background: isTourTimingsActive ? '#22c55e' : 'rgba(148, 163, 184, 0.5)',
                                  boxShadow: isTourTimingsActive ? '0 0 8px #22c55e' : 'none',
                                }}
                              />
                              <span>{isTourTimingsActive ? '⏱️ Master Schedule (ON)' : '⏱️ Set Schedule / Time'}</span>
                            </button>
                          )
                        })()}
                      </>
...
{isP2Rep && <span className="schedule-player-reported-badge">✓ Reported</span>}
      ...
      {/* Bottom Action Strip at the End of Fixtures Draw Sheet (Organizer / Admin Fixtures Only) */}
      {!isPublicView && (
        <div
          style={{
            marginTop: '32px',
      ...
      {!isPublicView && quickScoreScheduleMatch && (() => {
        ...
        return (
          ...
          >
            ...
            >
              {/* Header */}
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '14px', borderBottom: '1px solid rgba(148, 163, 184, 0.2)', paddingBottom: '10px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                  <span style={{ fontSize: '12px', fontWeight: '800', background: '#2563eb', color: '#ffffff', padding: '3px 8px', borderRadius: '6px' }}>
              ...
              {/* Set-by-Set Score Inputs */}
              {!isByeMatch ? (
                <div style={{ background: '#1e293b', borderRadius: '14px', padding: '16px', marginBottom: '16px', border: '1px solid rgba(148, 163, 184, 0.2)' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', color: '#94a3b8', fontSize: '11px', fontWeight: '800', textTransform: 'uppercase', marginBottom: '10px', padding: '0 4px' }}>
                    <span>{p1?.name || 'Player 1'} (Left)</span>
                    <span>Set Scores</span>
                    <span>{p2?.name || 'Player 2'} (Right)</span>
                  </div>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                    {setsArr.map((s) => {
                      ...
                      const valA = targetMatch[keyA] !== undefined ? targetMatch[keyA] : ''
                      const valB = targetMatch[keyB] !== undefined ? targetMatch[keyB] : ''
                      ...
                      return (
                        ...
                        >
                          <span style={{ fontSize: '13px', fontWeight: '800', color: '#38bdf8', minWidth: '50px' }}>
                            Set {s}:
                          </span>
                          ...
                          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                            ...
                            <span style={{ fontSize: '18px', fontWeight: '900', color: '#64748b' }}>-</span>
                          ...
                          <div style={{ display: 'flex', gap: '4px' }}>
                            <button
                              type="button"
                              onClick={() => {
                                handleScoreChange(targetMatch, keyA, 21)
                                handleScoreChange(targetMatch, keyB, valB !== '' ? valB : 15)
                              }}
                              title="Set 21 for Player 1"
                              style={{
                                background: 'rgba(56, 189, 248, 0.15)',
                                border: '1px solid rgba(56, 189, 248, 0.3)',
                                color: '#38bdf8',
                            ...
                            <button
                              type="button"
                              onClick={() => {
                                handleScoreChange(targetMatch, keyA, valA !== '' ? valA : 15)
                                handleScoreChange(targetMatch, keyB, 21)
                              }}
                              title="Set 21 for Player 2"
                              style={{
                ...
                <div style={{ padding: '14px', background: 'rgba(168, 85, 247, 0.1)', border: '1px solid rgba(168, 85, 247, 0.3)', borderRadius: '10px', marginBottom: '16px', textAlign: 'center' }}>
                  <span style={{ fontSize: '13px', fontWeight: '800', color: '#c084fc' }}>⚡ BYE Walkover Match</span>
                </div>
              )}
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
                  <button
                    ...
                    style={{
                      padding: '10px 12px',
                      borderRadius: '10px',
                      background: isP1Winner ? 'linear-gradient(135deg, #16a34a 0%, #15803d 100%)' : '#1e293b',
                      border: `1.5px solid ${isP1Winner ? '#22c55e' : 'rgba(56, 189, 248, 0.35)'}`,
                      color: isP1Winner ? '#ffffff' : '#60a5fa',
                      fontWeight: '800',
                      fontSize: '13px',
                      cursor: 'pointer',
                      display: 'flex',
                  ...
                  >
                    <span>{isP1Winner ? '🏆' : '👑'}</span>
                    <span>{p1?.name || 'Player 1'} {isP1Winner ? 'Won' : 'Winner'}</span>
                  </button>
                  ...
                  <button
                    ...
                    style={{
                      padding: '10px 12px',
                      borderRadius: '10px',
                      background: isP2Winner ? 'linear-gradient(135deg, #16a34a 0%, #15803d 100%)' : '#1e293b',
                      border: `1.5px solid ${isP2Winner ? '#22c55e' : 'rgba(244, 63, 94, 0.35)'}`,
                      color: isP2Winner ? '#ffffff' : '#f43f5e',
                      fontWeight: '800',
                      fontSize: '13px',
                      cursor: 'pointer',
                      display: 'flex',
                  ...
                  >
                    <span>{isP2Winner ? '🏆' : '👑'}</span>
                    <span>{p2?.name || 'Player 2'} {isP2Winner ? 'Won' : 'Winner'}</span>
                  </button>
                </div>
              </div>

              {/* Action Buttons */}
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderTop: '1px solid rgba(148, 163, 184, 0.2)', paddingTop: '14px' }}>
                <button
                  type="button"
        ...
        {isStreamActive && String(activeLiveMatchId) === String(selectedMatch?.id || selectedMatchId) ? (
          <>
            <button
              ...
              title={`TV Live Stream is Active. Click to view/reopen for "${formatTournamentName(selectedMatch?.matchName)}"`}
            >
              <span style={{ width: '8px', height: '8px', borderRadius: '50%', background: '#ffffff' }} />
              <span>🔴 Live: ON (TV Cast)</span>
            </button>
